//Maci's submission for Digikey Intro to RTOS Challenge 7 Semaphores
...
static SemaphoreHandle_t critical_mutex;
static SemaphoreHandle_t buf_taken_sem;
static SemaphoreHandle_t buf_empty_sem;
...
void setup() {
  ...
  Serial.println("Start Challenge 7: Semaphore");

  // Create mutexes and semaphores before starting tasks
  bin_sem = xSemaphoreCreateBinary();
  critical_mutex = xSemaphoreCreateMutex();
  buf_taken_sem = xSemaphoreCreateCounting(BUF_SIZE, 0);
  buf_empty_sem = xSemaphoreCreateCounting(BUF_SIZE, BUF_SIZE);
...
void producer(void *pvParameters) {
  ...
  for (int i = 0; i < num_writes; i++) {
    ...
    xSemaphoreTake(buf_empty_sem, portMAX_DELAY);
...
void consumer(void *pvParameters) {
  ...
  while (1) {
    ...
    xSemaphoreTake(buf_taken_sem, portMAX_DELAY);
